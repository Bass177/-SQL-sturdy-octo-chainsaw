...
SELECT product_name, units_in_stock, units_on_order, reorder_level, discontinued
FROM products
...
(	SELECT DISTINCT customer_id, COUNT(ship_via) AS shipper_count
	FROM orders
	GROUP BY customer_id
)
...
SELECT p.product, c.catrgory_name 
FROM products p 
...
SELECT territory_id, t.territory_description, r.region_description 
FROM territories t 
JOIN regions r ON t.region_id = r.region_id
