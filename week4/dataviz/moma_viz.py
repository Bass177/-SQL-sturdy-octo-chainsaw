...
def task1():
    ...
    query = """
    SELECT moma_works.department, COUNT(*)
    FROM moma_works
    GROUP BY moma_works.department
    ORDER BY COUNT(*) DESC   
    """
...
def task2():
    ...
    query = """
        SELECT moma_works.classification, COUNT(*)
    FROM moma_works
    GROUP BY moma_works.classification
    HAVING COUNT(*) > 100
    ORDER BY COUNT(*) DESC
    """
...
def task3():
    ...
    query = """
        ...
        FROM moma_artists 
        WHERE info ->> 'nationality' <> 'null'
    )
    SELECT nationality, COUNT(*) FROM  sample
...
def task4():
    ...
    query = """
        ...
        FROM moma_artists 
        WHERE info ->> 'gender' <> 'null'
    )
    SELECT UPPER(gender), COUNT(*) FROM  sample
